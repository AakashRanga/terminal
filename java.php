...
<body>
    ...
    <form method="post" action="">
        <label for="python_code">Enter Java Code:</label><br>
    ...
    <?php
     function a($command,$temp_file,$string){
        ...
        if (is_resource($process)) {
            ...
            if ((empty($output) && empty($errors)) || !empty($output)) {
                if($string=='JAVAC'){
                   
                    $command = escapeshellcmd("java " . $temp_file);
                    a($command,$temp_file,'JAVA');
                }else{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['python_code'])) {
        ...
        $temp_file = tempnam(sys_get_temp_dir(), 'Factorial') . '.java';
        file_put_contents($temp_file, $python_code);

        // Command to run the Python script
        $command = escapeshellcmd("javac " . $temp_file);
        a($command,$temp_file,'JAVAC'); 
