...
<body>
    ...
    <form method="post" action="">
        <label for="cpp_code">Enter C++ Code:</label><br>
        <textarea id="cpp_code" name="cpp_code" rows="10" cols="50"><?php echo isset($_POST['cpp_code']) ? htmlspecialchars($_POST['cpp_code']) : ''; ?></textarea><br>
        <button type="submit">Run C++ Code</button>
    </form>

    <h2>Output:</h2>
    <?php
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cpp_code'])) {
        $cpp_code = $_POST['cpp_code'];

        // Save the C++ code to a temporary file
        $temp_file = tempnam(sys_get_temp_dir(), 'cpp_code_') . '.cpp';
        $binary_file = tempnam(sys_get_temp_dir(), 'cpp_code_') . '.exe';
        file_put_contents($temp_file, $cpp_code);

        // Command to compile the C++ code
        $compile_command = "g++ " . escapeshellarg($temp_file) . " -o " . escapeshellarg($binary_file) . " 2>&1";
